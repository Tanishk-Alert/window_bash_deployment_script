...
create_application_services() {
    ...
    add_nssm_service_if_not_exists \
      ...
      '-cp "./lib/*" -Xms2g -Xmx4g -Dconfig.file=conf/application.conf -Dlogback.debug=true -Dorg.owasp.esapi.resources=conf -Dlog4j.configurationFile=conf/log4j2.xml play.core.server.ProdServerStart' \
      "$INIT_APPS_PATH/alert-api-server-1.0" \
    ...
    add_nssm_service_if_not_exists \
      "SVC_JOB" \
      "$JAVA_HOME/bin/java.exe" \
      '-cp "./lib/*" -Xms2g -Xmx4g -Dconfig.file=conf/jobserver.conf -Dhttp.port=9090 -Dlogback.debug=true -Dorg.owasp.esapi.resources=conf -Dlog4j.configurationFile=conf/log4j2.xml play.core.server.ProdServerStart' \
      "$INIT_APPS_PATH/alert-job-server-1.0" \
